    ...
    <Card className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="text-center">
          ...
          <p className="text-2xl font-bold">{(performance.overall_summary.execution_quality * 100).toFixed(0)}%</p>
        </div>
        <div className="text-center">
          <p className="text-sm text-muted-foreground">Decision Accuracy</p>
          <p className="text-2xl font-bold">{(performance.overall_summary.decision_accuracy * 100).toFixed(0)}%</p>
        </div>
        <div className="text-center">
          <p className="text-sm text-muted-foreground">Spacing Index</p>
          <p className="text-2xl font-bold">{(performance.overall_summary.spacing_index * 100).toFixed(0)}%</p>
        </div>
        <div className="text-center">
          <p className="text-sm text-muted-foreground">Transition Speed</p>
          <p className="text-2xl font-bold">{performance.overall_summary.transition_speed_sec.toFixed(1)}s</p>
        </div>
      </div>
